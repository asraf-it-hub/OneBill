-- OneBill Comprehensive Cloud Sync Migration
-- 1. Ensure all extended business profile columns exist on public.businesses
...
alter table public.businesses add column if not exists upi_name text;
alter table public.businesses add column if not exists invoice_notes text;
alter table public.businesses add column if not exists terms_and_conditions text;
alter table public.businesses add column if not exists logo_image text;
alter table public.businesses add column if not exists payment_qr_image text;

-- 2. Ensure public.invoice_items has business_id column
alter table public.invoice_items add column if not exists business_id uuid;

-- 3. Drop restrictive check constraints and foreign keys that cause sync failures
alter table public.customers drop constraint if exists customers_phone_check;
alter table public.invoices drop constraint if exists invoices_paid_paise_check;
alter table public.invoices drop constraint if exists invoices_discount_paise_check;
...
alter table public.invoices drop constraint if exists invoices_check2;
alter table public.invoices drop constraint if exists invoices_check3;
alter table public.invoices drop constraint if exists invoices_check4;
alter table public.invoices drop constraint if exists invoices_business_id_customer_id_fkey;
...
alter table public.invoice_items drop constraint if exists invoice_items_check1;
alter table public.invoice_items drop constraint if exists invoice_items_check2;
alter table public.invoice_items drop constraint if exists invoice_items_description_check;
alter table public.invoice_items drop constraint if exists invoice_items_quantity_milliunits_check;

-- 4. RPC to apply sync operations safely and idempotently
...
begin
  ...
  if op_type = 'BusinessCreated' then
    ...
    -- Auto-insert placeholder customer if not yet created so foreign key constraint never fails
    ...
    values (
      entity_id, p_owner_id, business_id, nullif(payload->'invoice'->>'customerId', '')::uuid, coalesce(payload->'invoice'->>'invoiceNumber', 'INV-001'),
      coalesce((payload->'invoice'->>'issuedAt')::timestamptz, now()), (payload->'invoice'->>'dueAt')::timestamptz,
      coalesce((payload->'invoice'->>'subtotalPaise')::bigint, 0), coalesce((payload->'invoice'->>'discountPaise')::bigint, 0),
      coalesce((payload->'invoice'->>'interestPaise')::bigint, 0), coalesce((payload->'invoice'->>'paidPaise')::bigint, 0),
      payload->'invoice'->>'notes', coalesce((payload->'invoice'->>'createdAt')::timestamptz, now()),
      coalesce((payload->'invoice'->>'updatedAt')::timestamptz, now()), (payload->'invoice'->>'deletedAt')::timestamptz
    )
    on conflict (id) do update set
      owner_id = excluded.owner_id,
    ...
    select coalesce(sum(amount_paise), 0) into payment_amount
    from public.payments where invoice_id = (payload->>'invoiceId')::uuid and deleted_at is null;
    update public.invoices set paid_paise = payment_amount, updated_at = now()
    ...
    values (
      ...
      payload->>'description', coalesce((payload->>'expenseDate')::timestamptz, now()), coalesce((payload->>'createdAt')::timestamptz, now()),
      coalesce((payload->>'updatedAt')::timestamptz, now()), (payload->>'deletedAt')::timestamptz
    )
    on conflict (id) do update set
      owner_id = excluded.owner_id,
  ...
  elsif op_type in ('InventoryProductCreated', 'InventoryProductUpdated', 'InventoryAdjusted') then
    ...
    values (
      entity_id, p_owner_id, business_id, coalesce(payload->>'name', ''), payload->>'sku', coalesce(payload->>'unit', 'pcs'),
    ...
    values (
      entity_id, p_owner_id, business_id, coalesce(payload->>'name', ''), coalesce(payload->>'phone', ''),
...
-- 5. RPC to fetch complete user snapshot
...
as $$
  select jsonb_build_object(
    'businesses', coalesce((select jsonb_agg(to_jsonb(b)) from public.businesses b where b.owner_id = auth.uid() and b.deleted_at is null), '[]'::jsonb),
