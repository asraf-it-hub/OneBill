-- Add extended business profile columns to public.businesses
...
-- Update apply_sync_operation RPC to handle all profile fields
...
begin
  ...
  if op_type = 'BusinessCreated' then
    begin
      ...
      values (
        ...
        payload->>'address',
        payload->>'upiId',
        coalesce(payload->>'preferredLanguage','en'),
        payload->>'website',
        payload->>'tagline',
        payload->>'gstin',
        payload->>'upiName',
  ...
  elsif op_type in ('PaymentRecorded', 'PaymentReversed') then
    if op_type = 'PaymentRecorded' then
      ...
      select (payload->>'invoiceId')::uuid into business_id;
      if business_id is not null then
        select coalesce(sum(amount_paise), 0) into payment_amount
        from public.payments where invoice_id = business_id and deleted_at is null;
    ...
    if op_type = 'IncomeCreated' then
      insert into public.income_entries (id, owner_id, business_id, amount_paise, category, description, income_date, created_at, updated_at, deleted_at)
      values (
        entity_id,
        p_owner_id,
        business_id,
    ...
    if op_type = 'ExpenseCreated' then
      ...
      update public.expenses set
        amount_paise = (payload->>'amountPaise')::bigint,
        category = coalesce(payload->>'category', 'General'),
        description = payload->>'description',
    ...
    if op_type = 'InventoryProductCreated' then
      ...
      values (
        entity_id, p_owner_id, business_id,
        coalesce(payload->>'name', 'Product'),
        payload->>'sku',
    ...
    if op_type = 'SupplierCreated' then
      ...
      values (
        entity_id, p_owner_id, business_id,
        coalesce(payload->>'name', 'Supplier'),
    ...
    return jsonb_build_object('acknowledged', false, 'operationId', op_id, 'reason', 'Unsupported operation type');
  end if;
  ...
  begin
    ...
    values (op_id, p_owner_id, business_id, p_operation->>'entityType', entity_id, op_type, payload, p_device_id, coalesce((p_operation->>'occurredAt')::timestamptz, now()))
